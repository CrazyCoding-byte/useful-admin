...
void drawRect(SDL_Renderer *renderer, int x0, int y0, int w, int h, Color c) {
  ...
  drawLine(renderer, x0, y0, x1, y0, c); // 上边
  drawLine(renderer, x1, y0, x1, y1, c); // 右边
  drawLine(renderer, x1, y1, x0, y1, c); // 下边
  drawLine(renderer, x0, y1, x0, y0, c); // 左边
}
// 绘制实心矩阵
/**
...
int main(int argc, char *argv[]) {
  ...
  drawLine(renderer, 100, 100, 700, 100, green);
  // 绘制2：垂直直线（红色）：(100,100) → (100,500)（Δx=0）
  drawLine(renderer, 100, 100, 100, 500, red);
