...
//    @Override
    ...
    @Override
    public OrderConfirmVo findCartActivityAndCoupon(List<CartItemVo> cartInfoList,
                                                    Long userId) {
...
//        orderTradeVo.setCouponReduceAmount(couponReduceAmount);
    ...
    //获取购物车对应规则数据
    public List<CartInfoVo> findCartActivityList(List<CartItemVo> cartInfoList) {
        //创建最终返回集合
        List<CartInfoVo> cartInfoVoList = new ArrayList<>();
        //获取所有skuId
        List<Long> skuIdList = cartInfoList.stream().map(CartItemVo::getSkuId).collect(Collectors.toList());
        //根据所有skuId列表获取参与活动
        List<ActivitySku> activitySkuList = baseMapper.selectCartActivity(skuIdList);
        //根据活动进行分组，每个活动里面有哪些skuId信息
        //map里面key是分组字段 活动id  一个活动可以有多少sku
        ...
        //获取活动里面规则数据
        //key是活动id  value是活动里面规则列表数据  一个活动有多个规则 比如中秋节 满减30-2 满减50-5
        ...
        if (!CollectionUtils.isEmpty(activityIdSet)) {
            //activity_rule表  这里对规则的金额和数量进行了排序
        ...
        if (!CollectionUtils.isEmpty(activityIdToSkuIdListMap)) {
            ...
            while (iterator.hasNext()) {
                ...
                //获取当前活动对应的购物项列表
                List<CartItemVo> currentActivityCartInfoList = cartInfoList.stream()
                        .filter(cartInfo ->
                ...
                //计算活动对应规则
                //根据activityId获取活动对应规则  这里奇怪的是为什么不直接根据活动获取activiinfo的活动类型 而是通过活动规则表获取 但是无影响因为一个活动对应多个规则
        ...
        if (!CollectionUtils.isEmpty(skuIdList)) {
            //skuId对应购物项
            Map<Long, CartItemVo> skuIdCartInfoMap = cartInfoList.stream().collect(
                    Collectors.toMap(CartItemVo::getSkuId, CartInfo -> CartInfo)
            );
            for (Long skuId : skuIdList) {
                CartInfoVo cartInfoVo = new CartInfoVo();
                cartInfoVo.setActivityRule(null);//没有活动
                List<CartItemVo> cartInfos = new ArrayList<>();
    ...
    private ActivityRule computeFullDiscount(Integer totalNum, BigDecimal totalAmount, List<ActivityRule> activityRuleList) {
        ...
        if (null == optimalActivityRule) {
            //如果没有满足条件的取最小满足条件的一项 取 “门槛最低的规则”：因为列表按优惠力度从大到小排序，最后一个规则是门槛最低的（如 “满 2 件 9 折” 比 “满 3 件 8 折” 门槛低）。
            //目的是提示用户 “最容易达到的优惠条件”。
    ...
    private BigDecimal computeTotalAmount(List<CartItemVo> cartInfoList) {
        ...
        for (CartItemVo cartInfo : cartInfoList) {
            //是否选中
            if (cartInfo.getCheck()) {
                BigDecimal itemTotal = cartInfo.getPrice().multiply(new BigDecimal(cartInfo.getCount()));
    ...
    private int computeCartNum(List<CartItemVo> cartInfoList) {
        int total = 0;
        for (CartItemVo cartInfo : cartInfoList) {
            //是否选中
            if (cartInfo.getCheck()) {
                total += cartInfo.getCount();
            }
        }
        return total;
    }
...
//            List<SkuInfo> skuInfoList = productFeignClient.findSkuInfoList(activitySkus.stream().map(ActivitySku::getSkuId).collect(Collectors.toList()));
