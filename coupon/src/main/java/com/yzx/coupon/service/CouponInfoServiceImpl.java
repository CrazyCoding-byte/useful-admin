...
public class CouponInfoServiceImpl extends ServiceImpl<CouponInfoMapper, CouponInfo> implements CouponInfoService {
    ...
    //3 获取购物车可以使用优惠卷列表
    @Override
    public List<CouponInfo> findCartCouponInfo(List<CartItemVo> cartInfoList,
                                               Long userId) {
        //1 根据userId获取用户全部优惠卷
        ...
        for (CouponInfo couponInfo : userAllCouponInfoList) {
            ...
            if (CouponRangeType.ALL == couponInfo.getRangeType()) {
                ...
                //满足使用范围购物项
                List<CartItemVo> currentCartInfoList = cartInfoList.stream()
    ...
    //获取购物车对应优惠卷
    @Override
    public CouponInfo findRangeSkuIdList(List<CartItemVo> cartInfoList,
                                         Long couponId) {
        //根据优惠卷id基本信息查询
    ...
    //4 获取优惠卷id 对应skuId列表
    //优惠卷id进行分组，得到map集合
    private Map<Long, List<Long>> findCouponIdToSkuIdMap(List<CartItemVo> cartInfoList,
                ...
                for (CouponRange couponRange : rangeList) {
                    ...
                    if (couponRange.getRangeType() == CouponRangeType.SKU
                        ...
                        skuIdSet.add(cartInfo.getSkuId());
                            ...
                            && couponRange.getRangeId().longValue() == cartInfo.getCatalogId().longValue()) {
                        skuIdSet.add(cartInfo.getSkuId());
                    } else {
