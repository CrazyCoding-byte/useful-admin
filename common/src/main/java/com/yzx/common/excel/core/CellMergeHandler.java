...
import com.yzx.common.excel.annotation.CellMerge;
import com.yzx.common.utils.ReflectUtils;
...
public class CellMergeHandler {
    ...
    public List<CellRangeAddress> handle(List<?> rows) {
        ...
        for (Map.Entry<Field, FieldColumnIndex> item : mergeFields.entrySet()) {
            ...
            int colNum = itemValue.getColIndex();
            CellMerge cellMerge = itemValue.getCellMerge();
            ...
            for (int i = 0; i < rows.size(); i++) {
                ...
                if (!rowRepeatCellMap.containsKey(field)) {
                    rowRepeatCellMap.put(field, new RepeatCell(currentRowObjFieldVal, i));
                    continue;
                }
    ...
    private Map<Field, FieldColumnIndex> getFieldColumnIndexMap(Class<?> clazz) {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            mergeFields.put(field, new FieldColumnIndex(index, cm));
    ...
    private void appendMergeResult(List<CellRangeAddress> result, RepeatCell repeatCell, int endIndex, int colNum) {
        if (repeatCell == null || endIndex <= repeatCell.getCurrent()) {
            return;
        }
        result.add(new CellRangeAddress(repeatCell.getCurrent() + rowIndex, endIndex + rowIndex, colNum, colNum));
    }

    // ===================== Java8兼容 替换record 静态内部类 =====================

    /**
     * 单元格合并缓存实体（替代Java16+ Record）
     */
    ...
    /**
     * 字段列索引和合并注解信息（替代Java16+ Record）
     */
    public static class FieldColumnIndex {
        private final int colIndex;
        private final CellMerge cellMerge;

        public FieldColumnIndex(int colIndex, CellMerge cellMerge) {
