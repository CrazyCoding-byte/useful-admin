...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
    ...
    public static String padl(final String s, final int size, final char c) {
        ...
        if (s != null) {
            ...
            if (len <= size) {
                // JDK8 无String.repeat，循环拼接填充字符
                appendFillChar(sb, c, size - len);
                sb.append(s);
            } else {
                // 超长截取末尾size位
            ...
            // s为null，全填充指定字符
            appendFillChar(sb, c, Math.max(0, size));
        }
        return sb.toString();
    }

    /**
     * JDK8兼容：循环向StringBuilder拼接N个填充字符
     * @param sb 拼接容器
     * @param fillChar 填充字符
     * @param count 填充个数
     */
    private static void appendFillChar(StringBuilder sb, char fillChar, int count) {
